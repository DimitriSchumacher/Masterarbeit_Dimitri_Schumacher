...
def stat_test(alpha):
    ...
    for i in range(iterations):
        ...
        FUS_data.append(percentages_FUS[count:count+len(conditions)*len(time)])
        
    for i1 in range(iterations):
        
        x = FUS_data[i1]
        
        for i2 in range(len(conditions)*len(time)):
...
def plot_bars_FUS():
    ...
    fig.suptitle("GGGGG: Percentage of Cells with FUS Aggregate")
