...
cutoff1 = 0 # Max intensity cutoff to determine if a cell expressed enough FUS to be analysed
cutoff2 = 0 # Max intensity cutoff for antibody labelled G3BP
...
for i in range(len(coloc2_stats)):
    ...
    if "Coloc_Job_Name" in coloc2_stats[i]:
        ...
        if channel1_MaxInt > cutoff1 and channel2_MaxInt > cutoff2:
            ...
            elif "Vibration_1h" in name:
            
                data[2].append("60min Vib.")
                
            elif "1h_Vibration" in name:
            
                data[2].append("60min Vib.")
            
            elif "20min" in name:
            
                data[2].append("20min Vib.")
...
def data_stats_calculation():
    ...
    conditions = ["Control", "20min Vib.", "60min Vib."]
...
def plot_mean_pearson_individual():
    ...
    axs[0].bar(labels_p1[0:3], mean_p1[0:3], yerr=std_p1[0:3], capsize=8, color = ["c", "b", "r"])
    axs[0].set_ylabel("Mean Pearson's R (No Threshold)")
    axs[0].set_ylim(-1,1)
    axs[1].bar(labels_p1[3:6], mean_p1[3:6], yerr=std_p1[3:6], capsize=8, color = ["c", "b", "r"])
    axs[2].bar(labels_p1[6:9], mean_p1[6:9], yerr=std_p1[6:9], capsize=8, color = ["c", "b", "r"]) 
...
def plot_bar_scatter():
    ...
    axs[0].set_ylabel("Mean Pearson's R (No Threshold)")
    axs[0].set_ylim(-1,1)
    axs[0].bar(position, mean_p1[0:3], yerr=std_p1[0:3], width=w, capsize=8, color=(0,0,0,0), tick_label=labels_p1[0:3], edgecolor=["c", "b", "r"] )
    axs[0].scatter(position[0] + np.random.random(c[0].size) * w - w / 2, np.array(list(map(float, c[0]))), s=2)
    axs[0].scatter(position[1] + np.random.random(c[1].size) * w - w / 2, np.array(list(map(float, c[1]))), s=2)
    axs[0].scatter(position[2] + np.random.random(c[2].size) * w - w / 2, np.array(list(map(float, c[2]))), s=2)
    axs[1].bar(position, mean_p1[3:6], yerr=std_p1[3:6], width=w, capsize=8, color=(0,0,0,0), tick_label=labels_p1[3:6], edgecolor=["c", "b", "r"])
    axs[1].scatter(position[0] + np.random.random(c[3].size) * w - w / 2, np.array(list(map(float, c[3]))), s=2)
    axs[1].scatter(position[1] + np.random.random(c[4].size) * w - w / 2, np.array(list(map(float, c[4]))), s=2)
    axs[1].scatter(position[2] + np.random.random(c[5].size) * w - w / 2, np.array(list(map(float, c[5]))), s=2)
    axs[2].bar(position, mean_p1[6:9], yerr=std_p1[6:9], width=w, capsize=8, color=(0,0,0,0), tick_label=labels_p1[6:9], edgecolor=["c", "b", "r"]) 
    axs[2].scatter(position[0] + np.random.random(c[6].size) * w - w / 2, np.array(list(map(float, c[6]))), s=2)
    axs[2].scatter(position[1] + np.random.random(c[7].size) * w - w / 2, np.array(list(map(float, c[7]))), s=2)
    axs[2].scatter(position[2] + np.random.random(c[8].size) * w - w / 2, np.array(list(map(float, c[8]))), s=2)
    for x in range(len(axs)):
        axs[x].tick_params(axis="x", rotation=70)
        axs[x].yaxis.grid(True)
    plt.show()
...
def plot_violinplots():
    ...
    fig, axs = plt.subplots(1,3, sharey=True)
    fig.suptitle("Pearson's R Values (No Threshold)")
    axs[0].set_ylabel("Pearson's R Distribution (No Threshold)")
...
def plot_boxplots():
    ...
    for mutant in mutations: 
        ...
        for condition in conditions: 
            ...
            axs[count].boxplot(list(map(float, b[3].values)), positions=[pos[count2]], showfliers=False)
