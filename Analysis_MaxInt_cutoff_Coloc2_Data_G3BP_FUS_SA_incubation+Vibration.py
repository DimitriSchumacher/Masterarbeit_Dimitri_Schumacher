...
def plot_bar_scatter():
    ...
    axs[0].set_ylabel("Mean Pearson's R (No Threshold)")
    axs[0].set_ylim(-1,1)
    axs[0].bar(position, mean_p1[0:2], yerr=std_p1[0:2], width=w, capsize=8, color=(0,0,0,0), tick_label=labels_p1[0:2], edgecolor=["b","r"] )
    ...
    axs[1].bar(position, mean_p1[2:4], yerr=std_p1[2:4], width=w, capsize=8, color=(0,0,0,0), tick_label=labels_p1[2:4], edgecolor=["b","r"])
    axs[1].scatter(position[0] + np.random.random(c[2].size) * w - w / 2, np.array(list(map(float, c[2]))), s=2)
    axs[1].scatter(position[1] + np.random.random(c[3].size) * w - w / 2, np.array(list(map(float, c[3]))), s=2)
    axs[2].bar(position, mean_p1[4:6], yerr=std_p1[4:6], width=w, capsize=8, color=(0,0,0,0), tick_label=labels_p1[4:6], edgecolor=["b","r"]) 
    axs[2].scatter(position[0] + np.random.random(c[4].size) * w - w / 2, np.array(list(map(float, c[4]))), s=2)
    axs[2].scatter(position[1] + np.random.random(c[5].size) * w - w / 2, np.array(list(map(float, c[5]))), s=2)
    for x in range(len(axs)):
        axs[x].tick_params(axis="x", rotation=70)
        axs[x].yaxis.grid(True)
    plt.show()
...
def plot_violinplots():
    ...
    for x in range(len(axs)):
        ...
        axs[x].set_xticklabels(labels_p1[0+x*2:3+x*2])
